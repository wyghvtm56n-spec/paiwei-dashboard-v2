...
<section class="message-hero"><span class="message-kicker">OFFICIAL MESSAGE CENTER</span><h1>三平台訊息，一個安全入口。</h1><p>統一檢視 LINE 官方帳號、Instagram 與 Facebook 私訊；保留原始訊息、客戶資料與客服工作區欄位，並以原始 D1 對話紀錄為準。</p><div class="footer-actions"><a class="message-action-link" href="${META_INBOX_URL}" target="_blank" rel="noreferrer">開啟 Meta Business Suite ↗</a><a class="message-action-link" href="${LINE_CONSOLE_URL}" target="_blank" rel="noreferrer">開啟 LINE 後台 ↗</a><a class="message-action-link" href="${INSTAGRAM_URL}" target="_blank" rel="noreferrer">開啟 Instagram ↗</a><button class="message-action-button" type="button" data-profile-refresh>同步 Facebook 聯絡人名稱</button><output class="reply-form-status" data-profile-refresh-status aria-live="polite"></output></div></section>
...
<div class="message-layout"><section class="message-card"><h2>原始對話與個資</h2><p>顯示既有 message_conversations、message_events、message_contact_profiles 與客服工作區；Messenger 與 Instagram 可在 24 小時標準視窗內直接回覆，LINE 仍由官方後台送出。</p><div class="conversation-list">${conversations.map((conversation, index) => renderConversation(conversation, events, index)).join("") || `<p class="empty-inline">目前沒有對話資料。</p>`}</div></section><aside class="message-card"><h2>通道與同步狀態</h2><p>資料更新時間：${escapeHtml(formatTime(data.fetchedAt))}</p><div class="platform-list">${renderPlatformSummary(platforms)}</div><div class="message-notice"><strong>隱私與資料邊界</strong><span>本頁位於管理密碼保護的 /messages 路由。原始聊天內容、顯示名稱、帳號名稱與客服欄位只在登入後顯示；不在公開首頁、影片 API 或網址中輸出。系統仍遵守既有 D1 retention 與加密 target binding 設計。</span></div><div class="message-notice"><strong>資料完整性</strong><span>本頁顯示最近 ${escapeHtml(formatNumber(conversations.length))} 個對話與 ${escapeHtml(formatNumber(events.length))} 則訊息事件。為避免 Worker 超過 subrequest 限制，資料由同一個 D1 查詢批次讀取。</span></div><div class="message-notice"><strong>Facebook 聯絡人識別</strong><span>按上方同步按鈕可分批透過 Meta User Profile API 補齊 Messenger 顯示名稱。此功能需要有效的 Page Token，以及 Meta Business Asset User Profile Access／Page「Info About People」權限；若 Meta 不提供 profile，系統會維持未識別，不猜測身份。</span></div><p class="panel-subtitle">未識別聯絡人：${escapeHtml(formatNumber(noName))}；可按「同步 Facebook 聯絡人名稱」逐批補齊，不會在此頁猜測身份。</p></aside></div>
</main><script>
document.querySelectorAll('[data-profile-refresh]').forEach(function (button) {
  button.addEventListener('click', async function () {
    var status = document.querySelector('[data-profile-refresh-status]');
    button.disabled = true;
    if (status) { status.className = 'reply-form-status'; status.textContent = '正在同步最近 10 筆 Facebook 聯絡人…'; }
    try {
      var response = await fetch('/messages/profile-refresh', {
        method: 'POST',
      ...
      if (!response.ok || !result.ok) throw new Error(result.error || '聯絡人同步未完成。');
      var guidance = result.guidance ? ' ' + result.guidance : '';
      if (status) status.textContent = '本次完成 ' + result.ready + ' 筆，未完成 ' + result.unavailable + ' 筆。' + guidance;
      window.setTimeout(function () { window.location.reload(); }, 1200);
    } catch (error) {
      button.disabled = false;
      if (status) { status.className = 'reply-form-status error'; status.textContent = error && error.message ? error.message : '聯絡人同步未完成。'; }
    }
  });
});
